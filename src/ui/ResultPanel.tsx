...
function WalkStep({
  ...
  return (
    <div className={`step ${state}`}>
      ...
      <div className="step-body">
        <div className="step-t1">
          {last ? `${leg.from.name} → 도착지까지 걷기` : `${leg.to.name} 까지 걷기`}
        </div>
        <div className="step-t2">
      ...
      <div className="steps">
        {plan.legs.map((leg, i) => {
          ...
          return leg.kind === "ride" ? (
            <RideStep key={i} leg={leg} state={state} />
          ) : (
            <WalkStep key={i} leg={leg} state={state} last={i === plan.legs.length - 1} />
          );
        })}
      </div>
      ...
      <div className="panel-scroll">
        {phase === "planning" && (
          <div className="note" style={{ alignItems: "center" }}>
            ...
            <div>
              {mode === "bus"
                ? "주변 정류장과 버스 노선을 훑는 중이에요… (처음엔 10초쯤 걸려요)"
                : "최적 경로를 계산하는 중이에요…"}
            </div>
          </div>
        )}
        ...
        {phase === "error" && error && (
          <>
            ...
            <button className="cta ghost" onClick={reset} style={{ marginTop: 12 }}>
              닫기
            </button>
          </>
        )}
